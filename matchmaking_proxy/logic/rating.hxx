...
size_t inline ratingShareLosingTeam (size_t userRating, std::vector<size_t> const &userRatings, size_t ratingChange)
{
...
size_t inline averageRating (std::vector<std::string> const &accountNames)
{
...
size_t inline ratingShareWinningTeam (size_t userRating, std::vector<size_t> const &userRatings, size_t ratingChange)
{
...
long inline ratingChange (size_t userRating, size_t otherUserRating, long double score, size_t ratingChangeFactor)
{
...
size_t inline averageRating (std::vector<size_t> const &ratings)
{
...
size_t inline averageRating (size_t sum, size_t elements) { return boost::numeric_cast<size_t> (std::rintl (boost::numeric_cast<long double> (sum) / elements)); }
auto constexpr SCORE_WON = 1;
auto const SCORE_DRAW = boost::numeric_cast<long double> (0.5);
auto constexpr RATING_CHANGE_FACTOR = 20;
std::pair<std::vector<database::Account>, std::vector<database::Account>> inline calcRatingLoserAndWinner (std::vector<database::Account> losers, std::vector<database::Account> winners)
{
...
std::vector<database::Account> inline calcRatingDraw (std::vector<database::Account> accounts)
{
