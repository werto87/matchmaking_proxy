...
connectWebsocket (io_context &ioContext, boost::asio::ip::tcp::endpoint const &endpoint, std::filesystem::path const &pathToSecrets, std::vector<std::string> &messagesFromGame)
{
  try
    {
      using namespace boost::asio;
      ...
      try
        {
          ...
          co_await(myWebsocket->readLoop ([myWebsocket, &ioContext, &messagesFromGame] (const std::string &msg) {
...
TEST_CASE ("server", "[integration]")
{
  ...
  SECTION ("start connect create account join game leave", "[matchmaking]")
  {
    ...
    co_spawn (io_context, connectWebsocket (io_context, userEndpoint, pathToSecrets, messagesFromGamePlayer1), printException);
    auto messagesFromGamePlayer2 = std::vector<std::string>{};
    co_spawn (io_context, connectWebsocket (io_context, userEndpoint, pathToSecrets, messagesFromGamePlayer2), printException);
    io_context.run ();
